...
Page({
  ...
  onLoad: function (options) {
    ...
    if (accountID != undefined && accountID != -1 && !isSelf) {
      ...
      // this.initCreatedList(true, true)
  ...
  changeTabs(e) {
    ...
    if (e.detail.activeKey == 'one') {
      ...
      wx.lin.renderWaterFlow([], true);
      setTimeout(function () {
        wx.lin.renderWaterFlow(have_square_list, true);
      }, 200)
    }
  },
  changeTabs1(e) {
  ...
  onShow: function () {
    ...
    if (this.data.account_info.account_id != undefined && !isSelf) {
      console.log("?????")
      this.initOtherCreateList(this.data.account_info.account_id, true, true)
      this.initOtherWantList(this.data.account_info.account_id, true)
      this.initOtherHaveList(this.data.account_info.account_id, true)
      this.initOtherSquareList(this.data.account_info.account_id, true)
    } else {
      this.initCreatedList(true, true)
      this.initWantList(true)
      this.initHaveList(false, true)
  ...
  onPullDownRefresh: function () {
    ...
    if (this.data.now_key == 'one') {
      createPageNum = 0
      if (isSelf) {
        this.initCreatedList(false, true)
      } else {
        this.initOtherCreateList(this.data.account_info.account_id, false, true)
      }
  ...
  initCreatedList(fisrt, showToast) {
    if (this.data.now_key == 'one') {
  ...
  initOtherSquareList(e, showToast) {
    ...
    let _this = this
    wx.request({
      url: prefix + '' + e + '/have_square_list?page_num=' + squarePageNum, //仅为示例，并非真实的接口地址
      method: 'GET',
      header: header,
      success(res) {
        //隐藏loading 提示框
        ...
        if (_this.data.now_key == 'four') {
          ...
          if (squarePageNum == 1) {
            have_square_list = newList
            wx.lin.renderWaterFlow([], true);
            setTimeout(function () {
              wx.lin.renderWaterFlow(newList, true);
            }, 100)
          } else {
            have_square_list = have_square_list.concat(newList)
            wx.lin.renderWaterFlow(newList, false);
          }
  ...
  updateList() {
    if (this.data.now_key == 'one') {
      if (isSelf) {
        this.initCreatedList(false, false)
      } else {
        this.initOtherCreateList(this.data.account_info.account_id, false, false)
      }
    } else if (this.data.now_key == 'two') {
      if (isSelf) {
        this.initWantList(false)
      } else {
        this.initOtherWantList(this.data.account_info.account_id, false)
      }
    } else if (this.data.now_key == 'three') {
      if (isSelf) {} else {
        this.initOtherSquareList(this.data.account_info.account_id, true)
